...
assert_exists "$ROOT_DIR/systems/laptop-mbpM2/users/rikki/meta.nix" "Darwin host 应保留清晰的用户实例数据入口"
assert_exists "$ROOT_DIR/systems/laptop-mbpM2/users/rikki/default.nix" "Darwin host 应保留清晰的用户补充模块入口"
...
assert_contains "$ROOT_DIR/systems/laptop-mbpM2/meta.nix" "import ./users/rikki/meta.nix" "Darwin host 的 meta 应显式导入用户数据"
assert_contains "$ROOT_DIR/systems/laptop-mbpM2/default.nix" "./users/rikki/default.nix" "Darwin host 的默认模块应显式导入用户补充模块"
